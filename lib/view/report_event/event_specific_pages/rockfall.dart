...
class Rockfall extends StatelessWidget {
  Rockfall({Key? key}) : super(key: key);
  final ReportEventController _reportEventController=Get.find();
  // We use same function for Rockfall and Landslide because both are exactly same.
  final double _inputFieldBorderRadius = 4;
  final List<Map<String,String>> _typeOfRockfallDamage=[
    {
                  ...
                  children: [
                    Column(
                      ...
                      children: [
                        Padding(
                          ...
                          child: Text("${tr("monitor_event.landslide.displaced_rock")}:",style: TextStyle(fontSize: 14,color: appTheme.iconColor),),
                        ),
                        Slider(
                    ...
                    Align(
                      ...
                      child: Wrap(
                        spacing: 12,
                        runSpacing: 12,
                        children: List.generate(_typeOfRockfallDamage.length, (index) {
                          return GestureDetector(
                            onTap: (){
                              _reportEventController.selectObservedDamageForLandslide(value: _typeOfRockfallDamage[index]["type_of_damage"]!);
                            },
                            child: Container(
                              height: 86,
                              ...
                              decoration: BoxDecoration(
                                ...
                                // border: Border.all(color: appTheme.inputFieldsBorderColor)
                                border: _reportEventController.landslideDamage["landslide_observed_damage"].contains(_typeOfRockfallDamage[index]["type_of_damage"]) ? Border.all(color: appTheme.primaryActionColor,width: 2) : Border.all(color: appTheme.inputFieldsBorderColor),
                              ),
                              padding:const EdgeInsets.all(12),
                              child: Row(
                                children: [
                                  Image.asset(_typeOfRockfallDamage[index]["icon_path"]!),
                                  const SizedBox(width: 12,),
                                  Expanded(child: Text(_typeOfRockfallDamage[index]["localization_ref"]!,style: TextStyle(fontSize: 14,color: appTheme.primaryTextColor),).tr())
                                ],
                              ),
                            ),
                          );
